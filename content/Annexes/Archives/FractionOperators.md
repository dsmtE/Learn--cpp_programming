...
Transformer la fonction `display` en surcharge de l'opérateur `<<` pour pouvoir afficher une fraction avec `std::cout` ([opérateurs de flux](/Lessons/Fundamentals/Operators/#opérateurs-de-flux)).
...
- Réécrire les opérateurs `+`, `-`, `*` et `/` en utilisant les opérateurs affectation afin d'éviter de dupliquer le code ([deux-en-un](/Lessons/Fundamentals/Operators/#deux-en-un)) (Commenter l'ancienne version des opérateurs par exemple)
