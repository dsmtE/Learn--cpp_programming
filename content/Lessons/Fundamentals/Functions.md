...
Les règles de nommage des fonctions sont les mêmes que pour nommer nos **variables**, vous pouvez aller les relire **[ici](/Lessons/Fundamentals/Variables/#quelques-règles-de-nommage)** si nécessaire.
...
Cela coupe le flot d’exécution de la fonction comme avec les mots-clés ```break``` et ```continue``` avec les [boucles](/Lessons/Fundamentals/ConditionsAndLoops/#contrôler-lexécution).
:::
